...
namespace G4.Plugins.Common.Transformers
{
    [G4Plugin(
        ...
        manifest: $"G4.Plugins.Common.Transformers.Manifests.{nameof(TrimEnd)}.json")]
    public class TrimEnd(G4PluginSetupModel pluginSetup) : PluginBase(pluginSetup)
    {
        protected override PluginResponseModel OnSend(PluginDataModel pluginData)
        {
            ...
            // Determine the transformed value by trimming specified characters from the end if provided.
            // If "TrimChars" has characters, trim those; otherwise, trim whitespace only.
            var transformerResult = trimChars.Length > 0
                ? originalValue.TrimEnd(trimChars)
                : originalValue.TrimEnd();
