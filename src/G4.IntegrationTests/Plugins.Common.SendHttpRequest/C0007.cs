    ...
    internal class C0007(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // Regular expression is applied to extract two-digit numbers from the HTTP response
                new ActionRuleModel
                {
                ...
                // Assert action: Asserts that the extracted number from the HTTP response
                // matches the specified value based on the assertion operation
                new ActionRuleModel
                {
                    PluginName = "Assert",
                    Argument = "{{$ --Condition:Text --Operator:" + assertionOperation + " --Expected:25}}",
