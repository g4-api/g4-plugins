    ...
    internal class C0001(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // RegisterParameter action: Registers the alert text as a session parameter
                new ActionRuleModel
                {
                    PluginName = "RegisterParameter",
                    Argument = "{{$ --Name:AlertText --Value:{{$" + pluginName + "}} --Scope:Session}}"
                },
                ...
                // with id "MacroResult" is equal to "Welcome to the Simulator Page"
                new ActionRuleModel
                {
                    PluginName = "Assert",
                    Argument = "{{$ " +
                        "--Condition:Attribute " +
                        "--Operator:Equal " +
                        "--Expected:Welcome to the Simulator Page}}",
                    OnAttribute = "value",
                    OnElement = "#MacroResult",
                    Locator = Locators.CssSelector
                }
