...
{
    internal class C0012(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // InvokeClick action: Invokes a click on the button element that contains the onclick attribute with 'ElementVisible' substring
                new ActionRuleModel
                {
                ...
                // WaitFlow action: Waits until the element with id "ElementVisible" is visible for a maximum of 15 seconds
                new ActionRuleModel
                {
                    PluginName = "WaitFlow",
                    Argument = "{{$ --Condition:ElementVisible --Timeout:00:00:15}}",
                    Locator = Locators.CssSelector,
                    OnElement = "#ElementVisible"
                },
                // Assert action: Asserts that the element with id "ElementVisible" is visible
