    ...
    internal class C0003(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // RegisterParameter action: Registers the window handle of the first tab obtained
                // from the Get-WindowHandle plugin with session scope
                new ActionRuleModel
                {
                    PluginName = "RegisterParameter",
                    Argument = "{{$ --Name:WindowHandle --Value:{{$Get-WindowHandle}} --Scope:Session}}"
                },
                // SendKeys action: Sends the window handle of the first tab obtained from the
                // specified plugin to the element with id "MacroResult"
                new ActionRuleModel
                {
                    PluginName = "SendKeys",
                ...
                // is not equal to the window handle of the first tab
                new ActionRuleModel
                {
                    PluginName = "Assert",
                    Argument = "{{$ " +
