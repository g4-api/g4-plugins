...
namespace G4.IntegrationTests.Plugins.Common.NewGuid
{
    internal class C0001(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<ActionRuleModel> OnActions(AutomationEnvironment environment)
        {
            ...
            var newGuidMacro = environment.TestParameters["macro"];
            var expectedPattern = environment.TestParameters["expectedPattern"];

            // Return a collection of action rule models
            return
            [
                // SendKeys action: Sends the new GUID macro to the element with ID "MacroResult"
                new ActionRuleModel
                {
                    PluginName = "SendKeys",
                    Argument = $"{newGuidMacro}",
