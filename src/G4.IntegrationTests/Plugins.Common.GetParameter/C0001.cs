...
namespace G4.IntegrationTests.Plugins.Common.GetParameter
{
    internal class C0001(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<ActionRuleModel> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // RegisterParameter action: Registers a parameter with the
                // expected value and specified scope
                new ActionRuleModel()
                {
                    PluginName = "RegisterParameter",
                    Argument = "{{$ --Name:ParameterName --Value:" + expected + " --Scope:" + scope + "}}",
                },
                // SendKeys action: Sends the value of the registered parameter
                // to an element with ID "MacroResult"
                new ActionRuleModel()
                {
                    PluginName = "SendKeys",
                    Argument = "{{$" + pluginName + " --Name:ParameterName --Scope:" + scope + "}}",
