...
{
    internal class C0037(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // SetCondition action: Sets a condition based on the visibility of the element
                // with the id "ElementNotVisible".
                new SwitchRuleModel
                {
                    PluginName = "SetCondition",
                    Argument = "{{$ --Condition:ElementVisible}}",
                    OnElement = "#ElementNotVisible",
                    Locator = "CssSelector",
