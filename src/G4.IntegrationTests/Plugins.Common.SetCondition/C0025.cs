...
{
    internal class C0025(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // SetCondition action: Sets a condition based on whether the element attribute
                // "title" of the element with ID "ElementAttribute" equals "AttributeElement".
                new SwitchRuleModel
                {
                    PluginName = "SetCondition",
                    Argument = "{{$ --Condition:ElementAttribute --Operator:Eq --Expected:AttributeElement}}",
