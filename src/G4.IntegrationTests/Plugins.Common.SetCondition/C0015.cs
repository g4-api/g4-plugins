...
namespace G4.IntegrationTests.Plugins.Common.SetCondition
{
    internal class C0015(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // SetCondition action: Sets a condition based on whether the text of the element
                // with id "ElementText" equals "ElementText".
                new SwitchRuleModel
                {
                    PluginName = "SetCondition",
                    Argument = "{{$ --Condition:ElementText --Operator:Eq --Expected:ElementText}}",
