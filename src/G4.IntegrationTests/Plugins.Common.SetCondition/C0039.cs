...
{
    internal class C0039(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // SetCondition action: Sets a condition based on the page URL not matching "(?i)conditions".
                new SwitchRuleModel
                {
                    PluginName = "SetCondition",
                    Argument = "{{$ --Condition:PageUrl --Operator:NotMatch --Expected:(?i)conditions}}",
