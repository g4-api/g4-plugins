    ...
    internal class C0003(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // HideSoftKeyboard action: Hides the soft keyboard by pressing the specified key code
                new ActionRuleModel
                {
                    Argument = "{{$ --Strategy:pressKey --KeyCode:" + environment.TestParameters.Get("keyCode", string.Empty) + "}}",
