    ...
    internal class C0005(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<ActionRuleModel> OnActions(AutomationEnvironment environment)
        {
            ...
            [
                ...
                // HideSoftKeyboard action: Hides the soft keyboard by pressing the specified key
                new ActionRuleModel
                {
                    Argument = "{{$ --Strategy:pressKey --Key:" + environment.TestParameters.Get("key", string.Empty) + "}}",
