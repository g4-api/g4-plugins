...
namespace G4.IntegrationTests.Plugins.Ui.SetCondition
{
    internal class C0018(TestContext context) : TestCaseBase(context)
    {
        protected override IEnumerable<G4RuleModelBase> OnActions(AutomationEnvironment environment)
        {
            ...
            return
            [
                // SetCondition action: Sets a condition based on whether the page title
                // equals "Element Conditions".
                new SwitchRuleModel
                {
                    PluginName = "SetCondition",
                    Argument = "{{$ --Condition:PageTitle --Operator:Eq --Expected:Element Conditions}}",
